...
static const char* UnicodeString2UTF8(PUNICODE_STRING pUnicodeString)
{
    ...
    int len = WideCharToMultiByte(CP_UTF8, 0, pUnicodeString->Buffer, pUnicodeString->Length / 2, szBuffer, sizeof(szBuffer), NULL, NULL);
    szBuffer[len] = '\0';
...
bool IsProcessNameBad(PUNICODE_STRING processName)
{
    ...
    {
        if (strequal(UnicodeString2UTF8(processName), BadProcessnameList[i]))
			return true;
    }
    return false;
}
...
bool IsWindowClassNameBad(PUNICODE_STRING className)
{
    ...
    {
		if (strequal(UnicodeString2UTF8(className), BadWindowClassList[i]))
			return true;
    }
    return false;
}
...
bool IsWindowNameBad(PUNICODE_STRING windowName)
{
    ...
    {
		if (strequal(UnicodeString2UTF8(windowName), BadWindowTextList[i]))
			return true;
    }
    return false;
}
...
static void GetBadObjectTypes()
{
	...
	auto dNtQuerySystemInformation = 
		exchange_data.lookup_trampoline<decltype(&NtQuerySystemInformation)>(&NtQuerySystemInformation);
    if (dNtQuerySystemInformation == nullptr)
        return;
    ...
    ULONG Size;
    NTSTATUS Status;
    if ((Status = dNtQuerySystemInformation(SystemExtendedHandleInformation,
                                            HandleInfo,
                                            sizeof(Dummy),
    ...
    Status = dNtQuerySystemInformation(SystemExtendedHandleInformation,
                                                    HandleInfo,
                                                    2 * Size,
                                                    nullptr);
    ...
    )
{
    ...
    if (RtlNtMajorVersion() > 6)
    {
        ...
        Status = dNtSetInformationProcess != nullptr
            ? dNtSetInformationProcess(ProcessHandle,
...
void * GetPEBRemote(HANDLE hProcess)
{
	...
	if (dNtQueryInformationProcess != nullptr)
	    dNtQueryInformationProcess = dNtQueryInformationProcess;
...
DWORD GetProcessIdByProcessHandle(HANDLE hProcess)
{
	...
	auto dNtQueryInformationProcess =
		exchange_data.lookup_trampoline<decltype(&NtQueryInformationProcess)>(&NtQueryInformationProcess);
	if (dNtQueryInformationProcess == nullptr)
		dNtQueryInformationProcess = NtQueryInformationProcess;
...
DWORD GetProcessIdByName(PUNICODE_STRING processName)
{
	...
	auto dNtQuerySystemInformation =
		exchange_data.lookup_trampoline<decltype(&NtQuerySystemInformation)>(&NtQuerySystemInformation);
	if (dNtQuerySystemInformation == nullptr)
		dNtQuerySystemInformation = NtQuerySystemInformation;
    ...
    NTSTATUS status = dNtQuerySystemInformation(SystemProcessInformation,
                                                systemProcessInfo,
                                                2 * size,
                                                nullptr);
    if (!NT_SUCCESS(status))
